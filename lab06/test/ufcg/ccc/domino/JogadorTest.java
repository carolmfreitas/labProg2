...
class JogadorTest {
	...
	@Test
	void testGetNomeJogador() {
		Jogador jogador = new Jogador("j", new JogaPrimeiraPossivel(), mao1);
		assertEquals("j",jogador.getNome());
	}
	
	@Test
	void testDecideJogada() throws EstrategiaInvalidaException {
		try {
			Jogador jogador = new Jogador("j", new JogaPrimeiraPossivel(), mao1);
			Mesa mesa = new Mesa();
			Jogada jogada = jogador.decideJogada(mesa);
			assertEquals(jogada,jogador.getUltimaJogada());
		} catch(EstrategiaInvalidaException e) {
	...
	@Test
	void testgetSomaNumPecas() {
		Jogador jogador = new Jogador("j", new JogaPrimeiraPossivel(), mao1);
		assertEquals(5,jogador.getSomaNumPecas());
	}
	
	@Test
	void testToString1() {
		Jogador jogador = new Jogador("j", new JogaPrimeiraPossivel(), mao1);
		assertEquals("j, mão: [0:2, 0:3]",jogador.toString());
	}
	...
	void testToString2() throws Exception {
		...
		Mesa mesa = new Mesa();
		Jogada jogada = jogador.decideJogada(mesa);
		assertEquals("j, joga: 0:2 na direita, mão: [0:2, 0:3]",jogador.toString());
	}
